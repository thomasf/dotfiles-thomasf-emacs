...
;; URL: http://paetzke.me/project/py-black.el
;; Package-Version: 20160925.1052
;; Version: 2016.1

;;; Commentary:

;; Provides the `py-black' command, which uses the external "autopep8"
;; tool to tidy up the current buffer according to Python's PEP8.

;; To automatically apply when saving a python file, use the
;; following code:

;;   (add-hook 'python-mode-hook 'py-black-enable-on-save)

;; To customize the behaviour of "autopep8" you can set the
;; py-black-options e.g.

;;   (setq py-black-options '("--max-line-length=100"))

;;; Code:

(defgroup py-black nil
  "Use autopep8 to beautify a Python buffer."
  :group 'convenience
  :prefix "py-black-")


(defcustom py-black-options nil
  "Options used for autopep8."
  :group 'py-black
  :type '(repeat (string :tag "option")))


(defun py-black--find-settings-path ()
  (when buffer-file-name
    (let ((dom-dir (locate-dominating-file buffer-file-name "pyproject.toml")))
      (when dom-dir
        (expand-file-name "pyproject.toml" dom-dir )))))

(defun py-black--call-executable (errbuf file)
  (let* ((pyproject-toml (py-black--find-settings-path))
         (pyproject-args (if pyproject-toml `("--config" ,pyproject-toml) nil)))
    (zerop (apply 'call-process "black" nil errbuf nil
                  (append py-black-options '("-q") pyproject-args `(,file))))))

;;;###autoload
(defun py-black ()
  "Deprecated! Use py-black-buffer instead."
  (interactive)
  (py-black-buffer))


;;;###autoload
(defun py-black-buffer ()
  "Uses the \"autopep8\" tool to reformat the current buffer."
  (interactive)
  (py-black-bf--apply-executable-to-buffer "black"
                                              'py-black--call-executable
                                              nil
                                              "py"))


;;;###autoload
(defun py-black-enable-on-save ()
  "Pre-save hook to be used before running autopep8."
  (interactive)
  (add-hook 'before-save-hook 'py-black-buffer nil t))
...
(defun py-black-bf--apply-rcs-patch (patch-buffer)
      ...
      (with-current-buffer patch-buffer
        ...
        (while (not (eobp))
          (unless (looking-at "^\\([ad]\\)\\([0-9]+\\) \\([0-9]+\\)")
            (error "invalid rcs patch or internal error in py-black-bf--apply-rcs-patch"))
          (forward-line)
...
(defun py-black-bf--replace-region (filename)
  (delete-region (region-beginning) (region-end))
  (insert-file-contents filename))


(defun py-black-bf--apply-executable-to-buffer (executable-name
                                           executable-call
                                           only-on-region
                                           file-extension)
  ...
  (let ((tmpfile (make-temp-file executable-name nil (concat "." file-extension)))
    ...
    (if (funcall executable-call errbuf tmpfile)
        (if (zerop (call-process-region (point-min) (point-max) "diff" nil
          ...
          (if only-on-region
              (py-black-bf--replace-region tmpfile)
            (py-black-bf--apply-rcs-patch patchbuf))
